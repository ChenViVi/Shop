...
<nav class="nav-extended pink accent-1">
    <div class="nav-wrapper">
        ...
        <a href="/admin/music/update/${music.id}" class="breadcrumb">修改</a>
...
<div class="row" style="margin: 20px; display: block;">
    <form:form action="/admin/music/updateP" method="post" commandName="musicP" role="form">
        <div class="input-field col s4">
            <input type="text" class="form-control" id="name" name="name" placeholder="输入标题:" value="${music.name}"/>
            <label for="name">名称:</label>
        </div>
        <div class="input-field col s4">
            <label for="score">评分:</label>
            <input type="text" class="form-control" id="score" name="score" placeholder="输入评分:" value="${music.score}"/>
        </div>
        <div class="form-group col s4">
            <label for="type.id">分类:</label>
            <select class="browser-default" id="type.id" name="type.id">
                <c:forEach items="${typeList}" var="type">
                    <c:if test="${type.id==music.type.id}">
        ...
        <div class="input-field col s6">
            ...
            <input type="text" class="form-control" id="cover" name="cover" placeholder="输入封面:" value="${music.cover}"/>
        </div>
        <div class="input-field col s6">
            <label for="link">链接:</label>
            <input type="text" class="form-control" id="link" name="link" placeholder="输入链接:" value="${music.link}"/>
        </div>
        <div class="input-field col s12">
            <label for="comment">简评:</label>
            <textarea class="materialize-textarea" id="comment" name="comment" placeholder="输入简评">${music.comment}</textarea>
        </div>
